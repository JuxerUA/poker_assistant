  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: GestureDetector(
          ...
          onVerticalDragStart: (_) { _needUpdateGameDrag = true; },
          onVerticalDragUpdate: (dragUpdateDetails) {
            if (dragUpdateDetails.delta.distance > 3 && _needUpdateGameDrag) {
              if (Poker.gameCount > 0) {
                if (DateTime.now().difference(Poker.startACurrentGameTime).inSeconds > 10) {
                  Poker.startNextGame();
                  bgColor = Poker.valueColor;
                  setState(() {});
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Poker.timeLeftSec > 0 ? Text('Start new round. ${(Poker.timeLeftSec / 60).round()} minutes left') : Text('Start new round. No time left'),
                ...
                ? FittedBox(
                  child: RotatedBox(
                    ...
                    child: Row(
                      ...
                      children: [
                        SizedBox(width: 50),
                        Text("${Poker.currentLittleBlind * 2}",
                          style: TextStyle(
                            fontSize: 250,
